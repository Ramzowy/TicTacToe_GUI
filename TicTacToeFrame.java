...
public class TicTacToeFrame extends JFrame {
                            ...
                            if (row != -1 && col != -1 && game.isValidMove(row, col)) {
                                ...
                                if (moves >= 5) {
                                    if (game.checkForWin(row, col)) {
                                        showWinnerDialog(); // Modified
                                        return;
                                    }
                                    if (moves == 9) {
                                        showTieDialog(); // Modified
                                        return;
                                    }
                                }
    ...
    private void showWinnerDialog() {
        ...
        int choice = JOptionPane.showConfirmDialog(this, "Player " + winner + " wins! Do you want to play again?", "Winner",
                JOptionPane.YES_NO_OPTION);
        if (choice == JOptionPane.YES_OPTION) {
            resetGame();
        } else {
    ...
    private void showTieDialog() {
        int choice = JOptionPane.showConfirmDialog(this, "The game is a tie! Do you want to play again?", "Tie",
                JOptionPane.YES_NO_OPTION);
        if (choice == JOptionPane.YES_OPTION) {
            resetGame();
        } else {
